...
async function getRolesMap() {
  try {
    ...
    return data.reduce((acc, r) => { acc[r.name] = r.id; return acc; }, {});
  } catch (e) {
    console.warn('[seed] Could not fetch roles:', e.message);
    return {};
  }
}
...
async function ensureSuperadmin() {
  ...
  const superadminRoleId = rolesMap.superadmin || null;
  const { data: profile, error: pErr } = await supabaseAdmin
    ...
    .single();
  if (pErr) throw pErr;
  console.log('[seed] Profile ensured (role_id:', superadminRoleId, ')');
}

ensureSuperadmin()
  .then(() => { console.log('✅ Superadmin seed complete'); process.exit(0); })
  .catch(err => { console.error('❌ Superadmin seed failed:', err.message); process.exit(1); });
